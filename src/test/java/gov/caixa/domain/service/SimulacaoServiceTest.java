...
class SimulacaoServiceTest {
        ...
        private ProdutoSelectorService produtoSelectorService;
        private SimulacaoRepository simulacaoRepository;
        private SimulacaoService sut;

        @BeforeEach
        void setUp() {
                produtoSelectorService = mock(ProdutoSelectorService.class);
                simulacaoRepository = mock(SimulacaoRepository.class);
                sut = new SimulacaoService(produtoSelectorService, simulacaoRepository);
        }
        ...
        void deveOrquestrarSimulacaoCompletoERetornarSimulacaoPersistida() {
                ...
                when(produtoSelectorService.selecionar("CDB", new BigDecimal("10000.00"), 12))
                                .thenReturn(produto);
                when(simulacaoRepository.salvar(any(Simulacao.class)))
                                .thenReturn(simulacaoSalva);

                // Act
                Simulacao resultado = sut.simular(123L, new BigDecimal("10000.00"), 12, "CDB");
                ...
                verify(produtoSelectorService).selecionar("CDB", new BigDecimal("10000.00"), 12);
